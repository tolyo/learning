...
object Anagrams {
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = {    
    def mergeSentenceOccurrences(occ: Occurrences): Occurrences = occ match {
	    case List() => List()
	    case x :: xs => 
	      (x._1, occ.filter(_._1 == x._1).map(_._2).sum) :: mergeSentenceOccurrences(xs.filter(_._1 != x._1))
	  }
    mergeSentenceOccurrences((for(x <- s) yield wordOccurrences(x)).flatten).sorted
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = 
    if(occurrences.isEmpty) List(List())
      ...
      x <- 0 to occurrences.head._2
      y <- combinations(occurrences.tail)
    } yield(occurrences.head._1, x) :: y).toList map (_ filter(_._2 != 0))
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = 
  	if(y.isEmpty) x
  	else y.toMap.foldLeft(x.toMap)((acc, kv) =>
  	  	if(acc.contains(kv._1) && acc.apply(kv._1) > kv._2) acc.updated(kv._1, acc.apply(kv._1) - kv._2)
  	  	else if(acc.contains(kv._1) && acc.apply(kv._1) == kv._2) acc - kv._1
  	  	else acc
  	  ).toList.sorted    
